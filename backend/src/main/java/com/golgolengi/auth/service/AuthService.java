...
public class AuthService {
    ...
    public TokenResponse devLogin(String name) {
        return findOrCreateAndIssue("dev", "dev_user_001", "dev@test.com", name, null);
    }

    public TokenResponse devReset(String name) {
        memberRepository.findBySocialProviderAndSocialId("dev", "dev_user_001")
                .ifPresent(member -> {
                    String memberId = member.getId().toHexString();
                    member.setOnboardingCompleted(false);
                    memberRepository.save(member);
                    healthProfileRepository.findByMemberId(memberId)
                            .ifPresent(healthProfileRepository::delete);
                    familyMemberRepository.findByMemberId(memberId)
